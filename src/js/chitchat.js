/**
 * @license
 * ChitChat 1.0.2 <https://enemetronics.com.ng/chitchat/>
 * By Sharon Abodunrin
...
(function(window) {
    ...
    const _log = window.createLogger('[ChitChat]');
    ...
    for (let i = 0x1F600; i <= 0x1F64F; i++) {
        emojis[0].push(String.fromCodePoint(i));
    }
    for (let i = 0x1F900; i <= 0x1F9FF; i++) {
        emojis[1].push(String.fromCodePoint(i));
    }
    for (let i = 0x1F300; i <= 0x1F5FF; i++) {
        emojis[2].push(String.fromCodePoint(i));
    }
    for (let i = 0x1F680; i <= 0x1F6FF; i++) {
        emojis[3].push(String.fromCodePoint(i));
    }
    for (let i = 0x1F1E6; i <= 0x1F1FF; i++) {
        for (let j = 0x1F1E6; j <= 0x1F1FF; j++) {
            emojis[4].push(String.fromCodePoint(i) + String.fromCodePoint(j));
        }
    }
    for (let i = 0x1F980; i <= 0x1F9FF; i++) {
        emojis[5].push(String.fromCodePoint(i));
    }
    ...
    let logArr = ['debug', 'error', 'log', 'warn'];
    let log = function () {
        for (var i = 0,len = arguments.length, args = new Array(len);i < len;i++) {
            args[i] = arguments[i];
        }

        _log[Array.isArray(args) ? 'apply' : 'call'](console, args);
        window.bridge.sendData({'type': 'log', 'log': 'default', 'args': args});
    };

    for (let i = 0;i < logArr.length;i++) {
        log[logArr[i]] = function () { 
            for (var j = 0,len = arguments.length, args = new Array(len);j < len;j++) {
                args[j] = arguments[j];
            }
            _log[logArr[i]][Array.isArray(args) ? 'apply' : 'call'](console, args);
            window.bridge.sendData({'type': 'log', 'log': logArr[i], 'args': args});
        }
    }
    ...
    function Notice(value) {
        ...
        app.toActivity({type: 'toast', text: value});
    }
    ...
    function Select(data) {
        let _this = this;
        this.input = EQuery.elemt('input', null, 'app-select-input', {name: data.name});
        ...
        for (let i = 0; i < data.select.length; i++) {
            this.option = EQuery.elemt('option', String(data.select[i]), null, {value: i});
    ...
    function Input(data) {
        ...
        if (data.type) type = data.placeholder;
    ...
    function openModal(app, data) {
        ...
        app.toActivity({type: 'color_change', color: 2});
        ...
        this.close = function() {
            ...
            app.toActivity({type: 'color_change', color: 0});
        };
    }
    ...
    function imageDrawer(app, file) {
        ...
        let currentMouseX = 0,
            ...
            currentColorIndex,
            colorContainers,
            previewTransparency = 0.65,
            availableThicknesses = [2, 3, 6, 10, 16, 28, 50],
            currentThicknessIndex,
            thicknessContainers,
            availableDrawTypes = [{ name: "Line", id: 1, continuous: true }],
        ...
        let optionContainer = this.toolbar = EQuery.elemt('div');
        ...
        setTimeout(function () {
            _this.canvas.setSize(_this.img.width, _this.img.height);
            _this.canvas2.setSize(_this.img.width, _this.img.height);
            _this.canvas.drawImage(_this.img, 0, 0);
            _this.history.push(_this.canvas2.imageData());
        }, 500);
        

        EQuery(this.canvas.domElement).css('position: absolute;top: 0;z-index:2');
        EQuery(this.canvas2.domElement).css('position: absolute;top: 0;z-index:1');
        window._image = this.img;
        window._canvas = this.canvas
        window._canvas2 = this.canvas2;

        EQuery(this.undoBtn).click(function () {_this.undo()});
        EQuery(this.redoBtn).click(function () {_this.redo()});

        this.undo = function () {
            if (this.history.length > 0) {
                this.redoStack.push(this.canvas2.imageData());
                const imageData = this.history.pop();
                this.canvas2.putImageData(imageData, 0, 0);
            }
        };

        this.redo = function () {
            if (this.redoStack.length > 0) {
                this.history.push(this.canvas2.imageData());
        ...
        let colorContainersBox = EQuery.elemt('div', null, null, null, 'margin: 4px; border: 1px solid #bbb; border-radius: 3px;');
        EQuery(optionContainer).addChild([colorContainersBox]);
        colorContainers = new Array(3 * 3 * 3);
        for (let i = 0; i < colorContainers.length; i++) {
            let color = availableColors[i] = [Math.floor((i % 3) * 255 / 2), Math.floor((Math.floor(i / 3) % 3) * 255 / 2), Math.floor((Math.floor(i / (3 * 3)) % 3) * 255 / 2), 1.0];
            let colorContainer = colorContainers[i] = EQuery.elemt('div', null, 'app-image-toolbar-btn', null, 'background-color: ' + rgb(color));
            EQuery(colorContainer).on('mousedown', (function(ix) {return function() {setColor(ix)}})(i), false);
            EQuery(colorContainersBox).addChild([colorContainer]);
        }
        let divClearLeft = EQuery.elemt('div', null, null, null, 'clear: left;');
        ...
        for (let i = 0; i < drawTypeContainers.length; i++) {
            let drawTypeContainer = drawTypeContainers[i] = EQuery.elemt('div', null, 'app-image-toolbar-btn');
        ...
        EQuery(optionContainer).addChild([thicknessContainersBox]);
        thicknessContainers = new Array(availableThicknesses.length);
        for (let i = 0; i < thicknessContainers.length; i++) {
            let thicknessContainer = thicknessContainers[i] = EQuery.elemt('div', null, 'app-image-toolbar-btn');
            EQuery(thicknessContainer).addChild([document.createTextNode(String(availableThicknesses[i]))]);
            EQuery(thicknessContainer).on('mousedown', (function(ix) {return function() {setThickness(ix)}})(i), false);
            EQuery(thicknessContainersBox).addChild([thicknessContainer]);
        }
        ...
        let canvasMouseDownHandler = window.canvasMouseDownHandler = function(e) {
            if (e.button == 0) {
                currentMouseX = e.offsetX - _this.canvas.domElement.offsetLeft;
                currentMouseY = e.offsetY - _this.canvas.domElement.offsetTop;
        ...
        EQuery(this.canvas.domElement).on('mousedown', canvasMouseDownHandler, false);

        let canvasMouseMoveHandler = window.canvasMouseMoveHandler = function(e) {
            let mouseX = e.offsetX - _this.canvas.domElement.offsetLeft;
            let mouseY = e.offsetY - _this.canvas.domElement.offsetTop;

            if (mouseDown) {
                let drawType = availableDrawTypes[currentDrawTypeIndex];

                if (drawType.continuous) {
                    let path = new Path(drawType.id, availableColors[currentColorIndex], availableThicknesses[currentThicknessIndex], currentMouseX, currentMouseY, mouseX, mouseY);
                    path.draw(_this.canvas2.ctx);
                    currentMouseX = mouseX;
                    currentMouseY = mouseY;
                } else {
                    let color = availableColors[currentColorIndex].slice(0);
                    color[3] = previewTransparency;
                    currentPreviewPath = new Path(drawType.id, color, availableThicknesses[currentThicknessIndex], currentMouseX, currentMouseY, mouseX, mouseY, false);
                    currentMouseX = mouseX;
                    currentMouseY = mouseY;
                }
                refreshDisplayCanvas();
        ...
        EQuery(document).on('mousemove', canvasMouseMoveHandler, false);

        EQuery(document).on('mouseup', function (e) {
            if (e.button == 0) {
                if (mouseDown) {
                    mouseDown = false;
                    currentPreviewPath = null;
        ...
        EQuery(_this.canvas.domElement).on('mouseout', function(e) {
            mouseInWindow = false;
            refreshDisplayCanvas();
        }, false);
        ...
        function refreshDisplayCanvas() {
            ...
            _this.canvas2.ctx.drawImage(_this.canvas.domElement, 0, 0);
            if (currentPreviewPath != null) {
                window.currentPreviewPath = currentPreviewPath
                currentPreviewPath.draw(_this.canvas2.ctx);
            } else if (mouseInWindow && !mouseDown) {
                _this.canvas.ctx.beginPath();
                let color = availableColors[currentColorIndex].slice(0);
                color[3] = previewTransparency;
                _this.canvas.ctx.fillStyle = rgb(color);

                _this.canvas.ctx.arc(currentMouseX, currentMouseY, availableThicknesses[currentThicknessIndex] / 2, 0, Math.PI * 2.0, true);
        ...
        function setThickness(thicknessIndex) {
            if (typeof currentThicknessIndex !== "undefined") EQuery(thicknessContainers[currentThicknessIndex]).css('border-color: transparent')
            currentThicknessIndex = thicknessIndex;
            EQuery(thicknessContainers[currentThicknessIndex]).css('border-color: #c3c3c3');
        }

        function setColor(colorIndex) {
            if (typeof currentColorIndex !== 'undefined') EQuery(colorContainers[currentColorIndex]).css('border-color: transparent');
            currentColorIndex = colorIndex;
            EQuery(colorContainers[currentColorIndex]).css('border-color: #c3c3c3');
        }
        ...
        this.projectImage = function () {
            _this.canvas2.drawImage(_this.img, 0, 0);
            const imageData = this.history.pop();
            this.canvas2.putImageData(imageData, 0, 0);
            return _this.canvas2.dataUrl();
        };
    ...
    function loginPanel(elt) {
        ...
        this.type = EQuery.elemt('input', null, null, {type: 'hidden', value: 'chitchat-login', name: 'type'});
    ...
    function openLogin(app) {
        ...
        function subLogin() {
            ...
            fetch(host + 'chitchat/login/', {
                ...
                offline = true;
                log.error('Error requesting data: null');
    ...
    function openRegister(app, tab) {
        ...
        if (tab == 5) {
            send_email();
            [panel.dotContainer, panel.bottom].forEach(elt => {
                EQuery(elt).remove();
            });
        }
        if (tab == 6) {
            ...
            if (pass) {
                ...
                fetch(host + 'chitchat/register/', {
                    ...
                    if (data.indexOf('success') > -1) {
                        ...
                        EQuery(panel.err).text('An error occurred while processing your request');
            ...
            if (panel.profileSelector.btn3.stats == 3) {
                ...
                fetch(host + 'chitchat/register/', {
                    ...
                    if (data.indexOf('success') > -1) {
                        ...
                        EQuery(panel.err).text('An error occurred, please try again later.');
                    } else {
                        EQuery(panel.err).text('An error occurred, please try again later.');
                    }
                })/*.catch(function () {
    ...
    function openForgetPsw(app, tab) {
        ...
        EQuery(panel.subBtn).click(function () {
            ...
            EQuery(spinner).spinner();
            let modal = openModal(app, { content: [{ 'm2': { html: spinner }}] });
            fetch(host + 'chitchat/forgot-psw', {
                method: 'POST',
    ...
    function showLogins(app, json) {
        let display = app.display;
        let addSvg = EQuery.svg();
        addSvg.setSize(50, 50);
        addSvg.setViewBox(0, 0,122.88,122.88);
        addSvg.path('M61.44,0A61.46,61.46,0,1,1,18,18,61.25,61.25,0,0,1,61.44,0ZM88.6,56.82v9.24a4,4,0,0,1-4,4H70V84.62a4,4,0,0,1-4,4H56.82a4,4,0,0,1-4-4V70H38.26a4,4,0,0,1-4-4V56.82a4,4,0,0,1,4-4H52.84V38.26a4,4,0,0,1,4-4h9.24a4,4,0,0,1,4,4V52.84H84.62a4,4,0,0,1,4,4Zm8.83-31.37a50.92,50.92,0,1,0,14.9,36,50.78,50.78,0,0,0-14.9-36Z', '#eee');
        ...
        let tab = EQuery.elemt('div', [loginLabel, pswInput, check], 'form-tab', null, 'display: block');

        let form = EQuery.elemt('form', [err, tab, type], 'regForm', {method: 'post'}, 'margin: 0;');

        let subBtn = EQuery.elemt('button', 'Submit', 'btn btn-padding-large btn-green e-mob-right');
        let btnContainer = EQuery.elemt('div', [subBtn], 'container', null, 'overflow: auto;padding: 0;position: absolute;bottom: 40px;width: calc(100% - 24px);');

        let qsw = EQuery.elemt('span', [EQuery.elemt('span', "Don't an account? "), EQuery.elemt('a', 'Register', null, {href: 'javascript:void(0)'})]);
        let psw = EQuery.elemt('span', [EQuery.elemt('a', 'Forgot Password?', 'e-mob-right', {href: 'javascript:void(0)'}, 'width: 121px')]);
        let bottom = EQuery.elemt('div', [qsw, psw], 'container', null, 'padding: 0;position: absolute;bottom: 15px;width: calc(100% - 24px);');

        let formCont = EQuery.elemt('div', [form, btnContainer, bottom], 'e-shadow', null, 'display: none;height: 100%;width: 100%;max-height: 550px;max-width: 400px;margin: 0 auto;background-color: rgb(51, 51, 51);border-radius: 12px;padding: 12px;');
        let pswCont = EQuery.elemt('div', [formCont], null, null, 'display: flex;height: 100%;width: 100%;margin-top: 10px;padding: 8px;');

        let addBtn = EQuery.elemt('li', [EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('div', [addSvg.domElement], 'imgContainer'), EQuery.elemt('div', [EQuery.elemt('h2', 'Add Account')], 'usernameContainer')], 'list-inner', null, 'display: flex')], 'list-container')]);
        let list = EQuery.elemt('ul', null, 'app-login-list');
        let listCont = EQuery.elemt('div', [list], null, null, 'height: 100%');
        let loginsList = EQuery.elemt('div', [listCont], null, null, 'padding: 0px 8px;height: 100%;');

        display.login = EQuery.elemt('div', [EQuery.elemt('div', 'Select an account to login:', null, null, 'font-size: 16px;text-align: center;'), EQuery.elemt('div', [
            EQuery.elemt('div', [loginsList], 'split-view-view visible app-col app-half top-small', null, 'height: 100%'),
            EQuery.elemt('div', [pswCont], 'split-view-view visible app-col app-half bottom-small', null, 'height: 100%')
        ], 'app-split-container', null, 'max-height: 348px')], 'clearfix', {id: 'login-list-container'}, 'overflow: auto;width: 100%;height: calc(100% - 81px)');
        EQuery(display.loginContainer).removeChildren();
        EQuery(display.loginContainer).addChild([EQuery.elemt('div', [EQuery.elemt('img', null, 'app-logo', {src: './assets/logo_img2.png'})], 'app-logo-container', null, 'text-align: center'), display.login]);

        for (let id in json) {
            let userData = json[id].userData, img = json[id].userData.profile_pic, username = json[id].userData.username;

            let imgCont = EQuery.elemt('div', [EQuery.elemt('img', null, null, {src: img}, '')], 'imgContainer');
            let profileCont = EQuery.elemt('div', [EQuery.elemt('h2', username)], 'usernameContainer');
            let elt = EQuery.elemt('div', [EQuery.elemt('div', [imgCont, profileCont], 'list-inner', null, 'display: flex')], 'list-container');

            EQuery(list).addChild([EQuery.elemt('li', [elt], null, {'data-list-id': id})]);
        }
        EQuery(list).addChild([addBtn]);

        EQuery(addBtn).click(function () {
            openLogin(app);
        });

        EQuery(list).find('li[data-list-id]').click(function () {
            let id = Number(EQuery(this).attr('data-list-id'));
            EQuery(list).find('li').removeClass('active');
            EQuery(this).addClass('active');
            EQuery(formCont).show();
            if (isMobile) EQuery(display.login).find('div.bottom-small').addClass('fixed-top');
            EQuery(pswInput).on('keydown', function (e) {
                if (e.key == 13) {
                    e.stopPropagation();
                    e.preventDefault();
                    submitForm(id);
            ...
            EQuery(subBtn).click(function () {
                submitForm(id);
            });
        });

        EQuery(checkPsw).click(function () {
            if (EQuery(pswInput).attr('type') == 'password') EQuery(pswInput).attr({type: 'text'});
            else EQuery(pswInput).attr({type: 'text'});
        });
        ...
        function submitForm(id) {
            ...
            EQuery(spinner).spinner();
            let modal = openModal(app, { content: [{ 'm2': { html: spinner }}] });
            fetch(host + 'chitchat/request.php', {
                method: 'POST',
                body: new FormData(EQuery.elemt('form', [EQuery.elemt('input', null, null, {name: 'type', value: 'login_with_id'}), EQuery.elemt('input', null, null, {name: 'id', value: id}), EQuery.elemt('input', null, null, {name: 'psw', value: pswInput.value}), ], null, {method: 'post'}))
            }).then((response)=>response.text()).then((data)=>{
                modal.close();
                if (data[0] == '{') {
                    let rawData = JSON.parse(data);
                    ...
                    EQuery(err).text('An error occured while processig your request');
                } else if (data == 'err974032') {
                    EQuery(err).text('Incorect Password!');
                } else if (data == 'err7934827') {
                    EQuery(err).text('No such account. Please Register');
                } else {
                    EQuery(err).text('An error occoured, try again later');
                }
                offline = false;
                updateDisplay(display);
            }).catch(()=>{
                offline = true;
                log.error('Error requesting data: null');
                EQuery(err).text('Check your internet connection');
                Notice('Bridge Error 1: Failed to connect to sserver');
                modal.close();
                updateDisplay(display);
            });
    ...
    function Storage() {
        ...
        let name = 'enemetronics-chitchat';
        let version = 1;
        let database;
        return {
    ...
    function Config() {
        ...
        let storage = {language: 'en',  exportPrecision: 6};
        ...
        if (window.localStorage) {
            ...
            return {getKey: function () {}, setKey: function () {}, clear: function () {}}
        }
    }
    ...
    function addEventListeners(app) {
        ...
        EQuery(display.sidebarContent).find('li').on('mouseenter', function() {
            let top = EQuery(this).findPosition().top + 12 - (isElectron ? 32 : 0), left = 51;
        ...
        EQuery(display.splitSideBar).find('#add_friend').click(function() {
            var modal = openModal2(app, {
                header: 'Add Friends',
                content: [{m12: {err: new Text('')}}, {m12: {text: new Text('Enter the username or user id to add')}}, {m12: {input: new Input({placeholder: 'Username/UserId'})}}, {m12: {html: EQuery.elemt('div')}}],
                btns: {
                    Search: searchFriend,
            ...
            EQuery(modal.input.input).on('keydown', function (e) {
                if (e.keyCode == 13) searchFriend(modal);
            });
        });
    ...
    function searchFriend(panel) {
        ...
        if (lowerC && upperC) {
            fetch(host + 'chitchat/request.php?type=friend_search&id=' + input).then(response => response.text()).then(data => {
                if (data[0] == '[') {
                ...
                log.error('Cannot fetch requested data');
            });
        } else {
            fetch(host + 'chitchat/request.php?type=friend_search&name=' + input).then((response)=>response.text()).then((data)=>{
                if (data[0] == '[') {
                    app.addFriend(JSON.parse(data), panel);
                } else if (data == 'err8279830') {
    ...
    function setChunks(text) {
        let q = [];
        if (text.length >= 500) {
            for (var i = 500;i <= text.length;i += 500) {
                q.push(text.slice(i - 500, i))
            }
        } else {
            q.push(text);
        }
    ...
    function textChunks(text) {
        let _this = this;this.chunks = setChunks(text);this.count = 1;

        this.more = EQuery.elemt('span', ' Read more...', null, null, 'color: #1089d3;cursor: pointer');
        this.content = EQuery.elemt('span', null, null, null, 'word-wrap: break-word;')
        this.elt = EQuery.elemt('pre', [this.content, this.more]);
            EQuery(this.content).text(this.chunks[0]);
        if (this.chunks.length == 1) {
            EQuery(this.more).hide();
        } else {
            EQuery(this.more).click(function () {
                if (_this.count < _this.chunks.length) {
                    _this.count++;
                    _this.content.innerText += _this.chunks[_this.count - 1];
                    if (_this.count == _this.chunks.length) EQuery(this).hide();
                } else {
                    EQuery(this).hide();
                }
            });
    ...
    function offlineElt() {
        ...
        return EQuery.elemt('div', [offlineSvg.domElement, EQuery.elemt('div', 'Network unavailable', null, null, 'margin-left: 12px;margin-top: 2px;color: #e2f1f1')], null, null, 'display: flex;padding-left: calc((100% - 158px)/2);min-width: 190px;');
    }
    ...
    function appendSideList(app) {
        ...
        for (let i in friends_arr) {
            if (i == 'pending') {}
                ...
                if (msg.status !== 'New Friend') {
                    content = EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('img', null, null, {src: imgUrl})], 'img-container ' + status,)], null, null, 'width: 45px;height: 100%;padding: 2px;'),EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('h2', username)], null, null, 'width: calc(100% - 45px);height: 60%;padding: 6px;display: flex;align-items: center;'),EQuery.elemt('div', [EQuery.elemt('div', (msg.fromUser == true ? 'You: ' : '') + msg.msg, null, null, 'font-size: 12px;width: 100%;height: 100%;display: block;clip: auto;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;font-size: 12px;opacity: .6;' + (msg.pending ? 'animation: fade-msg 2s ease-in infinite;' : ''))], 'l3', null, 'z-index: 2;position: absolute;bottom: 0px;width: calc(100% - 60px);height: 44%;padding: 3px;display: flex;'),EQuery.elemt('div', 'Typing...', 'typing-elt l3', null, 'z-index: 1;position: absolute;bottom: 0;width: calc(100% - 60px);height: 44%;padding: 6px;display: none;align-items: center;opacity: .6;')], null, null, 'height: 100%;width: 100%')], 'app-sidelist-content');
                } else {
                    content = EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('img', null, null, {src: imgUrl})], 'img-container ' + status)], null, null, 'width: 45px;height: 100%;padding: 2px;'),EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('h2', username)], null, null, 'width: calc(100% - 45px); height: 60%; padding: 6px; display: flex; align-items: center;'),EQuery.elemt('div', [EQuery.elemt('div', 'Start Chatting', null, null, 'font-style: italic;opacity: 0.4')], 'l3', null, 'z-index: 2; position: absolute; bottom: 0px; width: calc(100% - 60px); height: 44%; padding: 5px; display: flex; align-content: center;'),EQuery.elemt('div', 'Typing...', 'typing-elt l3', null, 'z-index: 1;position: absolute;bottom: 0;width: calc(100% - 60px);height: 44%;padding: 6px;display: none;align-items: center;opacity: .6;')], null, null, 'height: 100%;width: 100%')], 'app-sidelist-content');
                }

                let item = EQuery.elemt('div', [content], 'app-sidelist-item');
                EQuery(scroller).addChild([EQuery.elemt('li', [item], null, {'data-user-id': id, 'data-last-session': msg.time || 0})]);
            ...
            if (JSON.stringify(app.data[app.selectedID].msg) !== '{}') {
                if (!(app.data[app.selectedID].msg[id][app.data[app.selectedID].msg[id].length - 1] == undefined)) {
                    ...
                    if (msg.id == app.data[app.selectedID].userData.id) {
                        return {
                            msg: msg.txt.replaceAll('CHATAPP_SINGLE_QUOTE', '\'').replaceAll('CHATAPP_BACKSLASH', '\\'),
                            fromUser: true,
                            pending: msg.pending,
                            time: msg.time
                        };
        ...
        let list = EQuery(scroller).find('li[data-last-session]');
        for (let i = 0;i < list.length - 1;i++) {
            let currentList = list[i]
            let nextList = list[i + 1];
            let currentSession = toTime(EQuery(currentList).attr('data-last-session'));
            let nextSession = toTime(EQuery(nextList).attr('data-last-session'));
            
            if (nextSession > currentSession) {
                EQuery(currentList).before([nextList]);
            }
        }

        function toTime(date) {
            return (typeof date !== 'number' ? date.indexOf(':') !== -1 ? new Date(date).getTime() : Number(date) : date);
        }
    ...
    function Display(app) {
        ...
        this.outerContainer = EQuery.elemt('div', [this.splitContainer], 'split-view-view visible', null, 'position: absolute;top: 0px;height: 100%;transition: all 0.5s;z-index: 4;');
        ...
        this.container = EQuery.elemt('div', [this.outerSplitContainer, this.preview, this.contextView, this.overlay, this.loginContainer], 'sash-container scrollable-element clearfix', null, 'position: relative;height: 100%;width: 100%;');
        this.titleBar = EQuery.elemt('div', [EQuery.elemt('div', null, 'titlebar-drag-area'), EQuery.elemt('div', [EQuery.elemt('a', null, 'window-icon'), EQuery.elemt('div', 'ChitChat', 'title')], 'titlebar-left'), EQuery.elemt('div', null, 'titlebar-center'), EQuery.elemt('div', null, 'titlebar-right')], 'app-titlebar');
        this.mainSplit = EQuery.elemt('div', [EQuery.elemt('div', [this.titleBar], 'split-view-view' + (isElectron ? ' visible' : ''), null, 'position: absolute;top: 0;left: 0;height: 32px;'), EQuery.elemt('div', [this.container], 'split-view-view visible', null, 'position: absolute;left: 0;' + (isElectron ? 'top: 32px;height: calc(100% - 32px)' : 'top: 0px;height: 100%'))], 'app-split-container');
        this.wrapper = EQuery.elemt('div', [this.mainSplit], (isMobile ? 'mobile-view ' : '') + 'clearfix', {id: 'container'}, 'display: flex;background-color: rgb(27, 24, 34);min-height: 400px;');

        Sidebar(d, app);
        Main(d, app);
        updateDisplay(d);
        log('Initiizing display');
    }
    ...
    function chatElt(app, id) {
        ...
        emojiSvg.setViewBox(-12.5, -12.5, 150.88, 150.88);
        emojiSvg.path('M45.54,2.11A61.42,61.42,0,1,1,2.11,77.34,61.42,61.42,0,0,1,45.54,2.11Z', null, {'stroke': fill, 'stroke-width': 10});
        emojiSvg.path('M45.78,32.27c4.3,0,7.79,5,7.79,11.27s-3.49,11.27-7.79,11.27S38,49.77,38,43.54s3.48-11.27,7.78-11.27Z', null, {'stroke': fill, 'stroke-width': 10});
        emojiSvg.path('M77.1,32.27c4.3,0,7.78,5,7.78,11.27S81.4,54.81,77.1,54.81s-7.79-5-7.79-11.27S72.8,32.27,77.1,32.27Z', null, {'stroke': fill, 'stroke-width': 10});
        emojiSvg.path('M28.8,70.82a39.65,39.65,0,0,0,8.83,8.41,42.72,42.72,0,0,0,25,7.53,40.44,40.44,0,0,0,24.12-8.12,35.75,35.75,0,0,0,7.49-7.87.22.22,0,0,1,.31,0L97,73.14a.21.21,0,0,1,0,.29A45.87,45.87,0,0,1,82.89,88.58,37.67,37.67,0,0,1,62.83,95a39,39,0,0,1-20.68-5.55A50.52,50.52,0,0,1,25.9,73.57a.23.23,0,0,1,0-.28l2.52-2.5a.22.22,0,0,1,.32,0l0,0Z', null, {'stroke': fill, 'stroke-width': 10});
        ...
        this.infoBottom = EQuery.elemt('div', [EQuery.elemt('span', [EQuery.elemt('b', username), document.createTextNode(' is typing')], 'txt'), EQuery.elemt('div', null, 'dot'), EQuery.elemt('div', null, 'dot'), EQuery.elemt('div', null, 'dot')], 'app-chat-info', null, 'display: none');
        ...
        let sendMsg = this.sendMsg = function () {
            if (_this.chatInput.value !== '') {
                ...
                let src = app.data[app.selectedID].userData.profile_pic,
                    username = app.data[app.selectedID].userData.username,
                    txt = _this.chatInput.value, time = new Date();

                    if (app.sourceId == Number(app.selectedID)) {
                        app.newSource = false;
                        app.currentSource = true;
                    } else {
                        app.newSource = true;
                        app.currentSource = false;
                        app.sourceId = Number(app.selectedID);
                    }
                ...
                let body = EQuery.elemt('div', [EQuery.elemt('div', [new textChunks(txt).elt])], 'app-chat-body');
                ...
                let elt_ = EQuery.elemt('div', [EQuery.elemt('div', [(app.newSource ? head : EQuery.elemt('div')), body, foot], 'app-chat-msg-inner')], 'app-chat-msg pending from-sender' + (app.newSource ? ' new-source' : ''), {'data-chat-id': chatId, 'data-send-time': time.getTime()});
                EQuery(_this.chat).addChild([elt_]);
                _this.chat.scrollTo({top: _this.chat.clientHeight});
                
                _this.chatInput.value = '';

                fetch(host + 'chitchat/request.php', {
                    method: 'POST',
                    body: new FormData(EQuery.elemt('form', [EQuery.elemt('input', null, null, {name: 'sender_id', value: app.selectedID}), EQuery.elemt('input', null, null, {name: 'receiver_id', value: id}), EQuery.elemt('input', null, null, {name: 'msg', value: txt.replaceAll('\n', 'CHATAPP_NEWLINE')}), EQuery.elemt('input', null, null, {name: 'chat_id', value: chatId}), EQuery.elemt('input', null, null, {name: 'send', value: 'msg'})], null, {method: 'post'}))
                }).then(response => response.text()).then(data => {
                    if (data.indexOf('success') > -1) {
                        log('Message sent!');
                        appendSideList(app)
                        for (let i = 0;i < app.data[app.selectedID].msg[id].length;i++) {
                            let msg = app.data[app.selectedID].msg[id][i];
                            if (msg.msgData == chatId) {
                                msg.pending = false;
                                EQuery(_this.chat).find('.app-chat-msg[data-chat-id=' + chatId + ']').removeClass('pending');
                                app.save();
                                appendSideList(app);
                                return;
                            }
                ...
                app.socket.emit('message', {type: 'send_msg', sender: app.selectedID, receiver: id, msg: txt, chatId: chatId});
            } else {
                log.warn('Input empty. Message request will be discarded.');
            }
            EQuery('div.auto-reponse').remove();
        };
        
        EQuery(this.chatInput).on(['keypress', 'keyup', 'change'], function () {
            app.socket.emit('message', {type: 'typing', sender: app.selectedID, receiver: id});
        });
        ...
        EQuery(this.emojiBtn).click(function () {
            if (!isEmoji) {
                let main = EQuery.elemt('div', null, 'app-chat-emoji-main');
                let bottom = EQuery.elemt('div', null, 'app-chat-emoji-bottom');
                let inner = EQuery.elemt('div', [main, bottom], 'app-chat-emoji-inner');
                let elt = EQuery.elemt('div', [inner], 'app-chat-emoji');

                for (let i = 0;i < emojis.length;i++) {
                    let emojiBtn = EQuery.elemt('div', emojis[i][0]);
                    EQuery(emojiBtn).click(function () {
                        EQuery(bottom).find('div').removeClass('active')
                        EQuery(this).addClass('active');
                        EQuery(main).removeChildren();
                        for (let j = 0;j < emojis[i].length;j++) {
                            let btn = EQuery.elemt('div', emojis[i][j]);
                            EQuery(btn).click(function () {
                                _this.chatInput.value += emojis[i][j];
                            });
                            EQuery(main).addChild([btn]);
                        }
                    });

                    EQuery(bottom).find('div').removeClass('active');
                    EQuery(bottom).find('div:first-child').addClass('active')
                    EQuery(main).removeChildren();
                    for (let j = 0;j < emojis[0].length;j++) {
                        let btn = EQuery.elemt('div', emojis[0][j]);
                        EQuery(btn).click(function () {
                            _this.chatInput.value += emojis[0][j];
                        });
                        EQuery(main).addChild([btn]);
                    }

                    EQuery(bottom).addChild([emojiBtn]);
                }

                EQuery(_this.elt).addChild([elt]);
                isEmoji = true;
            } else {
                EQuery(_this.elt).find('.app-chat-emoji').remove();
                isEmoji = false;
            }
        });
        ...
        function formatText(txt) {
            ...
            function getNumPos(txt, func) {
                let arr = [' ', ';', '(', '+', ')', '[', ']', ',', '&', ':', '{', '}', '-', '*', '|', '%', '='], c, startpos = 0, endpos, word;
            ...
            let rest = txt, bold, italics, strike, monospace, info;

            for(;;) {
                bold = getPos(rest, '*', '*', function (s) {return EQuery.elemt('span', s, null, null, 'font-weight: bold')});
                italics = getPos(rest, '_', '_', function (s) {return EQuery.elemt('span', s, null, null, 'font-style: italics')});
                strike = getPos(rest, '~', '~', function (s) {return EQuery.elemt('span', s, null, null, 'font-style: strike')});
                monospace = getPos(rest, '\`', '\`', function (s) {return EQuery.elemt('span', s, null, null, 'font-family: monospace')});

                if (Math.max(bold[0], italics[0], strike[0], monospace[0]) == -1) { break; }
                let pos = getMinPos(bold, italics, strike, monospace);
                if (pos[0] == -1) { break; }
                if (pos[0] > -1) {
                    done += rest.substring(0, pos[0]);
                    done += pos[2](rest.substring(pos[0], pos[1]));
                    rest = rest.substr(pos[1]);
                }
            }
            
            info = extract(rest, '&gt;', '&lt;', function (s) {return EQuery.elemt('span', s, null, null, 'border-left: 1px solid #ccc;color: #ccc')});
            rest = info.rest;
    ...
    function App(elt) {
        ...
        // this.data = {'0': {userData: {status: '', username: '', id: 1, img: undefined, confirm_email: false, confirmSms: true}, msg: {}, contacts: {}, remember: false, logged_in: false, opt: {date_short: true}}};
        this.data = {};
        log(`ChitChat v ${this.version}.\n Start date: ${new Date()}`);
    ...
    App.prototype.fromJSON = function (json) {
        ...
        for (let id in json) {
            if (json[id].logged_in) {
                ...
                this.previewLoadScreen();
                this.beginUI(id)
                return;
            } else if (json[id].logged_in == undefined) {
                openLogin(app);
                return;
            }
        }
    ...
    App.prototype.fromActivity = function () {
        let _this = this;

        window.bridge.getArgs().then(args => {
            console.log(args)
            host = args.hostUrl || 'https://enemetronics.com.ng';
            port = args.wsPort || 8000;

            if (args.openChat) app.openChat(args.openChat)

        });

        window.bridge.getVersion().then(version => {
            if (version !== _this.version) {
                ...
                openModal2(this, {
                    content: [
                        ...
                        {'m12': {html: EQuery.elemt('div', [EQuery.elemt('div', 'You\'re using a version that is not supported.'), EQuery.elemt('div', `Client version: ${_this.version}\nServer version: ${version}`)])}}
                    ],
                    btns: {
                        'Okay': function () {
                            window.close();
    ...
    App.prototype.toActivity = function (json) {
        if (json.type == 'toast') {
            AndroidInterface.showToast(json.text)
        } else AndroidInterface.sendMessageToActivity(JSON.stringify(json));
    };

    App.prototype.fromSocket = function () {
        let _this = this, timeout1, timeout2;

        this.socket.on('version', v => {
            if (_this.version !== v) {
                log.warn(`Using an unsupported client with server\n Client Version: ${_this.version}\n Server Version: ${v}`);
                let svg = new EQuery.svg();
                svg.setSize(200, 200);
                svg.setViewBox(0, 0, 32, 32);
                svg.path('M16,30.5C8,30.5,1.5,24,1.5,16S8,1.5,16,1.5S30.5,8,30.5,16S24,30.5,16,30.5z M16,2.5   C8.6,2.5,2.5,8.6,2.5,16c0,7.4,6.1,13.5,13.5,13.5c7.4,0,13.5-6.1,13.5-13.5C29.5,8.6,23.4,2.5,16,2.5z M23.5,15.6c-0.3,0-0.5-0.2-0.5-0.5v-4.2c0-0.5-0.4-0.9-0.9-0.9H10.9c-0.5,0-0.9,0.4-0.9,0.9v1.6    c0,0.3-0.2,0.5-0.5,0.5S9,12.7,9,12.5v-1.6C9,9.8,9.8,9,10.9,9h11.3c1,0,1.9,0.8,1.9,1.9v4.2C24,15.4,23.8,15.6,23.5,15.6z M22.1,24H10.9c-1,0-1.9-0.8-1.9-1.9v-7.5c0-0.3,0.2-0.5,0.5-0.5s0.5,0.2,0.5,0.5v7.5c0,0.5,0.4,0.9,0.9,0.9    h11.3c0.5,0,0.9-0.4,0.9-0.9v-1.5c0-0.3,0.2-0.5,0.5-0.5s0.5,0.2,0.5,0.5v1.5C24,23.2,23.2,24,22.1,24z', '#aBABAB');
                ...
                openModal(this, {
                    content: [
                        {'m12': {html: EQuery.elemt('div', [svg.domElement], null, null, 'text-align: center')}},
                        {'m12': {html: EQuery.elemt('div', [EQuery.elemt('div', 'You\'re using a version that is not supported.'), EQuery.elemt('div', `Client version: ${_this.version}, Server version: ${v}`)])}}
                    ],
                    btns: {
                        'Okay': function () {
                            window.close();
        ...
        this.socket.on('user_list', json => {
            for (let contact in _this.data[_this.selectedID].contacts) {
                if (contact == 'pending') {}
                else if (contact == 'received') {}
                else {
                    _this.data[_this.selectedID].contacts[contact].status = 'offline';
                    for (let i = 0;i < json.length;i++) {
                        if (Number(contact) == Number(json[i].id)) {
                            _this.data[_this.selectedID].contacts[contact].status = json[i].status;
                        }
                    }
                    appendSideList(_this);
                }
            }
        });

        this.socket.on('check', msg => {
            if (msg == 'online') {
                _this.socket.emit('online_response', _this.selectedID);
            } else {}
        });

        this.socket.on('message', json => {
            if (json.type == 'typing') {
                for (let contact in app.data[app.selectedID].contacts) {
                    if (Number(json.receiver) == Number(app.selectedID) && Number(json.sender) == Number(contact) && contact !== 'pending' && contact !== 'received') {
                        clearTimeout(timeout1);
                        EQuery(_this.display.mainSideListTop).find('li[data-user-id=' + json.sender + '] .l3').css('display: none;');
                        EQuery(_this.display.mainSideListTop).find('li[data-user-id=' + json.sender + '] .typing-elt').css('display: flex;');
                        timeout1 = setTimeout(function () {
                            EQuery(_this.display.mainSideListTop).find('li[data-user-id=' + json.sender + '] .l3').css('display: flex;');
                            EQuery(_this.display.mainSideListTop).find('li[data-user-id=' + json.sender + '] .typing-elt').css('display: none;');
                        }, 2000);
                    }
                }
                if (_this.activeChat == json.sender) {
                    clearTimeout(timeout2);
                    EQuery('.app-chat-info').show();
                    timeout2 = setTimeout(function () {
                        EQuery('.app-chat-info').hide();
                    }, 2000)
                }
            } else if (json.type == 'send_msg') {
                if (Number(_this.selectedID) !== Number(json.sender) && Number(_this.selectedID) == Number(json.receiver)) {
                    _this.data[_this.selectedID].msg[json.sender].push({id: Number(json.sender), txt: json.msg, time: new Date().getTime(), pending: false, msgData: json.chatId});
                    _this.save();
                    appendSideList(_this);
                    if (Number(_this.activeChat) == Number(json.sender)) {

                        let txt = json.msg.replaceAll('CHATAPP_SINGLE_QUOTE', '\'').replaceAll('CHATAPP_BACKSLASH', '\\').replaceAll('CHATAPP_NEWLINE', '\n'),
                            username = app.data[app.selectedID].contacts[json.sender].username,
                            time = new Date().getTime(), src =  app.data[app.selectedID].contacts[json.sender].profile_pic;

                        if (app.sourceId == app.data[app.selectedID].contacts[json.sender].id) {
                            app.newSource = false;
                            app.currentSource = true;
                        } else {
                            app.newSource = true;
                            app.currentSource = false;
                            app.sourceId = app.data[app.selectedID].contacts[json.sender].id;
                        }

                        let head = EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('img', null, null, {src: src}), EQuery.elemt('div', username)], 'app-chat-head-inner')], 'app-chat-head');
                        let body = EQuery.elemt('div', [EQuery.elemt('div', [new textChunks(txt).elt])], 'app-chat-body');
                        let foot = EQuery.elemt('div', [EQuery.elemt('div', (app.data[app.selectedID].opt.date_short ? days_short : days)[new Date().getDay()] + ' ' + /\d\d:\d\d/.exec(Date(time))[0])], 'app-chat-foot');
                        let elt_ = EQuery.elemt('div', [EQuery.elemt('div', [(app.newSource ? head : EQuery.elemt('div')), body, foot], 'app-chat-msg-inner')], 'app-chat-msg from-receiver' + (app.newSource ? ' new-source' : ''), {'data-chat-id': json.chatId, 'data-send-time': time});

                        EQuery('div.auto-reponse').remove();
                        EQuery('.app-chatbox').addChild([elt_]);

                        EQuery('.app-chatbox')[0].scrollTo({top: EQuery('.app-chatbox')[0].clientHeight});
                    }
                }
            }
        });
    ...
    App.prototype.fetchData = function (elt) {
        let id = 0;
        log('Retriving updates from server...');
        ...
        fetch(host + 'chitchat/request.php', {
            ...
            if (data[0] == '{') {
                ...
                if (elt) {
                    EQuery(elt).css('animation: moveleft 0.5s ease-out');
                    setTimeout(function() {
                        EQuery(elt).remove();
                    }, 500);
                }
                ...
                log.warn('UserData could not be retrived. It could rather be a result of bad connection or server error.');
                log('Running initial userData'); 
            ...
            if (tries > 0) {
                tries--;
                setTimeout(function () {
                    log(`Reconnecting ${10 - tries} of 10`);
                    app.fetchData(elt);
                }, 5000);
            } else {
                log.warn('UserData could not be retrived. It could rather be a result of bad connection or server error.');
                log('Running initial userData'); 
                offline = true;
                log.warn('Cannot communicate with server');
    ...
    App.prototype.beginUI = function (id) {
        ...
        this.socket.emit('user_conn', {id: app.selectedID});
        this.fromSocket();
    };
    ...
    App.prototype.previewLoadScreen = function() {
        let _this = this;
        let display = this.display;
        let loadbar = EQuery.elemt('div', [EQuery.elemt('div')], 'loadbar')
        let text = EQuery.elemt('div', 'ChitChat', 'loadscreen-logo-text')
        let logo = EQuery.elemt('img', null, 'loadscreen-logo-img', {src: './assets/logo.png'});
        let logoContainer = EQuery.elemt('div', [EQuery.elemt('div', [logo, text], null, null, 'display: flex;align-items: center')], 'loadscreen', null, 'position: fixed;top: 50%;left: 50%;transform: translate(-50%, -50%);');
        let fullscreen = EQuery.elemt('div', [logoContainer], 'app-position-fixed app-fullscreen', {id: 'load-screen'}, 'background-color: #1c1c1c;z-index: 999');
        EQuery(display.container).addChild([fullscreen]);
        setTimeout(function() {
            EQuery(logoContainer).addChild([loadbar]);
            _this.fetchData(fullscreen);
        }, 3000);
    ...
    App.prototype.fromFile = function (file, filename, ext) {
        if (this.chat == true) {
            let isImage = imgTypes.indexOf(ext) > -1;
            ...
            let fileModal = openModal2(this, {
                ...
                content: [
                    ...
                    {m12: {html: isImage ? canvas.toolbar : EQuery.elemt('div')}},
                    {m12: {html: isImage ? canvas.elt : EQuery.elemt('div')}}
                ],
                btns: {
                    'Send': function (panel) {
        ...
        this.display.filePicker.value = '';
    };

    App.prototype.openChat = function(id) {
        let msg = this.data[app.selectedID].msg[id];
        let friends = this.data[app.selectedID].userData.friends.split(' ');
        let _this = this;
        this.chat = true;
        this.currentSource = false;
        this.sourceId = 0;
        ...
        if (friends.indexOf(id) == -1) {Notice('Contact is not part of friend list');log.warn('contact is not part of friend list');return;}
        ...
        if (msg) {
            for (let i = 0;i < msg.length;i++) {
                let txt = msg[i].txt.replaceAll('CHATAPP_SINGLE_QUOTE', '\'').replaceAll('CHATAPP_BACKSLASH', '\\').replaceAll('CHATAPP_NEWLINE', '\n'),
                    username = msg[i].id == app.data[app.selectedID].userData.id ? app.data[app.selectedID].userData.username : app.data[app.selectedID].contacts[id].username,
                    time = new Date(msg[i].time), 
                    src =  msg[i].id == app.data[app.selectedID].userData.id ? app.data[app.selectedID].userData.profile_pic : app.data[app.selectedID].contacts[id].profile_pic;

                if (app.sourceId == msg[i].id) {
                    app.newSource = false;
                    app.currentSource = true;
                } else {
                    app.newSource = true;
                    app.currentSource = false;
                    app.sourceId = msg[i].id;
                }
                    
                let head = EQuery.elemt('div', [EQuery.elemt('div', [EQuery.elemt('img', null, null, {src: src}), EQuery.elemt('div', username)], 'app-chat-head-inner')], 'app-chat-head');
                let body = EQuery.elemt('div', [EQuery.elemt('div', [new textChunks(txt).elt])], 'app-chat-body');
                let foot = EQuery.elemt('div', [EQuery.elemt('div', (app.data[app.selectedID].opt.date_short ? days_short : days)[time.getDay()] + ' ' + /\d\d:\d\d/.exec(time)[0])], 'app-chat-foot');
                let elt_ = EQuery.elemt('div', [EQuery.elemt('div', [(app.newSource ? head : EQuery.elemt('div')), body, foot], 'app-chat-msg-inner')], 'app-chat-msg' + (app.newSource ? ' new-source' : '') + (msg[i].id == app.data[app.selectedID].userData.id ? ' from-sender' : ' from-receiver') + (msg[i].pending ? ' pending' : ''), {'data-chat-id': msg[i].msgData, 'data-send-time': msg[i].time});
                EQuery(elt.chat).addChild([elt_]);
                elt.chat.scrollTo({top: elt.chat.clientHeight});
            }
            if (msg.length == 0) {
    ...
    App.prototype.addFriend = function (data, panel) {
        ...
        for (let i = 0;i < data.length;i++) {
            ...
            let item = EQuery.elemt('div', [content], 'app-sidelist-item');
            let container = EQuery.elemt('div', [item], 'e-box e-shadow', {'data-user-id': data[0].id, selected: true}, 'background-color: #333;border-radius: 12px;margin-bottom: 6px;cursor: pointer');
            ...
            EQuery(container).click(function () {
                if (id == app.selectedID) {
                    ...
                    fetch(host + 'chitchat/request.php', {
                        ...
                        } else if (data == 'err85082') {
                            Notice('You are already friends with this user');
                        } else {
    ...
    App.prototype.rejectFriend = function (id) {
        ...
        fetch(host + 'chitchat/request.php', {
            method: 'POST',
            body: new FormData(EQuery.elemt('form', [EQuery.elemt('input', null, null, {name: 'friend_id', value: id}), EQuery.elemt('input', null, null, {name: 'id', value: app.selectedID}), EQuery.elemt('input', null, null, {name: 'type', value: 'reject_friend_request'})], null, {method: 'post'}))
        }).then(response => response.text()).then(data => {
            if (data == 'success') {
    ...
    App.prototype.sendFile = function (id, file) {

    }

    App.prototype.logout = function () {
        app.data[app.selectedID].logged_in = false;
        app.reload();
    };
    ...
    App.prototype.reloadSide = function () {appendSideList(this)};

    App.prototype.updateDisplay = function () {updateDisplay(this.display)};
    ...
    app.storage.init(function() {
        ...
        app.fromActivity();
        getCountryNumbers();
        log('Reading data from indexedDB');
        app.storage.get(function(state) {
            ...
            log('Execution complete. Execution time: ' + (millis).toFixed(2) + 'ms.');
            (Math.floor(millis / 1000) > 5 ? log.warn('Can\'t keep up. Execution took more than 5 seconds!') : {});
        });
    });
